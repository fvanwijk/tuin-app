...
export const WeekTasksCard = ({ weekNumber, tasks, onTaskComplete, isCurrentWeek }: WeekTasksCardProps) => {
  return (
    <Card className={`${isCurrentWeek ? 'border-2 border-green-500' : ''}`}>
      <div className="flex items-center justify-between mb-4">
        <h2 className={`text-xl font-semibold ${isCurrentWeek ? 'text-green-600' : ''}`}>
          Week {weekNumber}
          {isCurrentWeek && <span className="ml-2 text-sm font-normal">(Huidige week)</span>}
        </h2>
                ...
                <div className="ml-3 flex-1">
                  <p
                    className={`font-medium ${
                      task.isCompletedThisYear ? 'line-through text-gray-500' : 'text-gray-900'
                    }`}
                  >
                    {task.title}
                  </p>
                  {task.description && (
                    <p
                      className={`text-sm ${task.isCompletedThisYear ? 'line-through text-gray-400' : 'text-gray-600'}`}
                    >
                      {task.description}
                    </p>
                  )}
                  <p className="text-xs text-gray-500 mt-1">Plant: {getPlantName(task)}</p>
                </div>
              </div>
            </li>
          ))}
